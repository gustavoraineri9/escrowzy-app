...
      // CORREÇÃO 1: Desestruturação mais robusta de getUser()
...
          tournament_id,
          status,
          joined_at, // <-- CORREÇÃO 2: Alterado de 'created_at' para 'joined_at'
          tournaments (
            id, 
            title, 
            game, 
...
            starts_at, 
            owner_id,
            profiles!tournaments_owner_id_fkey(id, full_name, avatar_url) // <-- CORREÇÃO 3: Perfil aninhado (Convite por quem criou o torneio)
          )
        `)
...
      if (participantError) {
        console.error("Erro ao buscar convites de torneio:", participantError);
        throw participantError;
      }

      for (const pInvite of participantInvites || []) {
        const tournament = pInvite.tournaments as any;
        // O perfil do CONVIDANTE (Owner) está aninhado em tournaments
        const profile = tournament?.profiles as any;

        if (tournament && profile) {
          // Busca o número atual de participantes (opcional, mas mantido)
...
            },
            createdAt: pInvite.joined_at, // <-- Usando o campo correto
...
          user_id,
          status,
          created_at, // 'created_at' está correto para a tabela 'friends'
...
      if (friendError) {
        console.error("Erro ao buscar solicitações de amizade:", friendError);
        throw friendError;
      }
...
    } catch (error) {
      console.error("Erro FATAL ao buscar convites:", error);
      toast({
        title: "Erro",
...
  // Funções handleAccept e handleDecline permanecem inalteradas
...
  // O JSX abaixo permanece inalterado, pois o problema era na busca de dados
  return (
