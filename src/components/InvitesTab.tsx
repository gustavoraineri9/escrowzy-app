    ...
    try {
      ...
      if (participantInvites) {
        const tournamentInvites: TournamentInvite[] = participantInvites.map((p: any) => ({
          ...
          type: "tournament",
          from: {
            name: p.tournaments.profiles.full_name || "Organizador",
            avatar: p.tournaments.profiles.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${p.tournaments.profiles.full_name}`,
            id: p.tournaments.profiles.id,
          },
          tournament: {
            id: p.tournaments.id,
            ...
            players: 0, // Placeholder, needs separate count
