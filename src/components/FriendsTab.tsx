...
interface AmigoExibicao extends Friend {
  estatisticas: HeadToHeadStats | null;
  online: boolean;
}
...
export const FriendsTab = () => {
  ...
  useEffect(() => {
    const buscarDadosAmigos = async () => {
      try {
        ...
        const amigosBuscados = await getFriends(user.id);
        const idsAmigos = amigosBuscados.map(a => a.friend_id);

        const promessasEstatisticas = idsAmigos.map(idAmigo => getHeadToHeadStats(user.id, idAmigo));
        const todasEstatisticas = await Promise.all(promessasEstatisticas);

        const mapaEstatisticas = new Map<string, HeadToHeadStats | null>();
        todasEstatisticas.forEach((estatisticas, indice) => {
          mapaEstatisticas.set(idsAmigos[indice], estatisticas || null);
        });
  ...
  const lidarComBusca = async () => {
    ...
    try {
      ...
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;
      const resultados = await searchUsers(termoBusca, user.id);
  ...
  const lidarComEnvioSolicitacao = async (idDestinatario: string) => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;
      await sendFriendRequest(user.id, idDestinatario);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {amigos.length === 0 && !carregandoAmigos ? (
          ...
          amigos.map((amigo) => {
            ...
            // adaptações para os nomes do serviço
            const suasVitorias = estatisticas?.wins_a || 0;
            const vitoriasDele = estatisticas?.wins_b || 0;
            const saldo = estatisticas?.balance_a || 0;
            const sequenciaVitorias = estatisticas?.win_streak_a || 0;
                ...
                <CardContent className="pt-6 pb-6 space-y-4">
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <Avatar className="w-14 h-14 border-2 border-primary/20">
                      ...
                      <AvatarFallback className="bg-primary/10 text-primary font-bold">
                        {currentUser?.user_metadata?.display_name?.substring(0, 2).toUpperCase() || currentUser?.user_metadata?.full_name?.substring(0, 2).toUpperCase() || "VC"}
                      </AvatarFallback>
                    </Avatar>
                    ...
                    <div className="relative">
                      <Avatar className="w-14 h-14 border-2 border-border">
                        <AvatarImage src={amigo.profiles?.avatar_url} />
                        <AvatarFallback className="bg-muted font-bold">
                          {amigo.profiles?.display_name?.substring(0, 2).toUpperCase() || amigo.profiles?.full_name?.substring(0, 2).toUpperCase()}
                        </AvatarFallback>
                      </Avatar>
                      {amigo.online && (
                  ...
                  <div className="text-center">
                    <div className="flex items-center justify-center gap-2">
                      <h3 className="font-bold text-lg">{amigo.profiles?.display_name || amigo.profiles?.full_name}</h3>
                      {amigo.online && (
...
export const FriendsTab = () => {
  ...
  const lidarComEnvioSolicitacao = async (idDestinatario: string) => {
    ...
    try {
      ...
      alert("Solicitação de amizade enviada!");
        const resultados = await searchUsers(termoBusca, currentUser.id);
      setTermoBusca("");
    } catch (err) {
      console.error("Erro ao enviar solicitação:", err);
      alert("Erro ao enviar solicitação de amizade.");
  ...
  const obterCorSaldo = (saldo: number) => {
    ...
    return "text-muted-foreground";
        ...
        await sendFriendRequest(currentUser.id, idDestinatario);
  const formatarSaldo = (saldo: number) => {
    if (saldo > 0) return `+ R$ ${saldo.toFixed(2)}`;
    if (saldo < 0) return `- R$ ${Math.abs(saldo).toFixed(2)}`;
    return `R$ ${saldo.toFixed(2)}`;
